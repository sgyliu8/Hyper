...
def edit_regions(window):
    ...
    target.addItems([item.text() for item in window.roi_names]); form.addRow('ROI', target)
    ...
    def load(index):
        record = window.regions()[index]; geometry = record['geometry']; kind = geometry['type']
        ...
        for spin,value in zip(bounds, window.rectangles()[index]):
    ...
    def apply():
        ...
        try:
            index = target.currentIndex(); record = deepcopy(window.regions()[index]); geometry = record['geometry']
            ...
            if checked['role'] == 'reference':
                window.reference_roi_id = checked['roi_id']
            window.rebuild_roi_graphics(); window.roi_changed()
    ...
    for text,step in [('Move up',-1),('Move down',1)]:
        ...
        def move(checked=False, delta=step):
            index=target.currentIndex(); other=index+delta
            if not 0 <= other < len(window.roi_names):
                return
            window.reorder_roi(index, other)
            target.blockSignals(True); target.clear(); target.addItems([item.text() for item in window.roi_names])
            target.setCurrentIndex(other); target.blockSignals(False); load(other)
