...
| Hardware deferred | Offline analysis is available; the current phase did not perform physical acceptance |
...
| Partial recording | Preserve the directory and failure reason; inspect accepted/written/explicitly_failed counts |
...
For developers, start with [the review matrix](../dev/REVIEW_PHASE3.md) and
[current handoff](../dev/HANDOFF.md), not an archived physical-recovery instruction.
